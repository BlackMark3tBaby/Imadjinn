...
def speech_recognition_worker(audio_queue, model_path="ai/models/vosk-model-small-en-us-0.15", sample_rate=44100):
    """
    Worker function that pulls audio from audio_queue and performs streaming ASR using Vosk.
    """
    if not os.path.exists(model_path):
        print(f"Vosk model not found at {model_path}. Please download and place it accordingly.")
        return
